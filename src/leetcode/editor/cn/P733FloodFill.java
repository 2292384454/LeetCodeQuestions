...
public class P733FloodFill {
    ...
    class Solution {
        ...
        public void bfs(int[][] image, int sr, int sc, int newColor, int originalColor) {
            int h = image.length, w = image[0].length;
            if (sr < 0 || sr >= h || sc < 0 || sc >= w || image[sr][sc] != originalColor) {
                return;
            }
            image[sr][sc] = newColor;
            bfs(image, sr + 1, sc, newColor, originalColor);
            bfs(image, sr - 1, sc, newColor, originalColor);
            bfs(image, sr, sc + 1, newColor, originalColor);
            bfs(image, sr, sc - 1, newColor, originalColor);
        }
    }
