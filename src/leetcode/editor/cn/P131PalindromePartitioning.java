...
public class P131PalindromePartitioning {
    ...
    class Solution {
        String str;//将输入字符串复制为成员变量
        boolean[][] dp;//动态规划数组
        List<List<String>> ans = new ArrayList<>();//最终结果
        ...
        public List<List<String>> partition(String s) {
            ...
            //动态规划找出所有的回文子串
            dp = new boolean[len + 1][len + 1];//dp[i][j]存储子串str[i,j)是不是回文串(j>=i)。
            //任意长度为0或1的子串都是回文串
            for (int i = 0; i < len; i++) {
                dp[i][i] = true;
                dp[i][i + 1] = true;
            ...
            for (int i = len - 2; i >= 0; i--) {
                for (int j = i + 2; j <= len; j++) {
                    dp[i][j] = (dp[i + 1][j - 1] && str.charAt(i) == str.charAt(j - 1));//str[i,j)是回文串当且仅当“str[i+1,j)是回文串且str[i]==str[j-1]”
                }
            }
