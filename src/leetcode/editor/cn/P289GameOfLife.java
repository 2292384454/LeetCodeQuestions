...
public class P289GameOfLife {
    ...
    class Solution {
        // 原始数据只用到了board的最低位，即第0位；
        // 用board的第1位存储更新结果，全部更新后右移一位得到最终结果
        public void gameOfLife(int[][] board) {
            int m = board.length, n = board[0].length;
            //开始更新
            for (int i = 0; i < m; i++) {
                for (int j = 0; j < n; j++) {
                    int count = liveNeiCount(board, i, j);
                    if (count == 2 || count == 3) {
                        // 如果活细胞周围八个位置有两个或三个活细胞，则该位置活细胞仍然存活；
        ...
        //统计(x,y)的邻居中活细胞的数量
        private int liveNeiCount(int[][] board, int x, int y) {
